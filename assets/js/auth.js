// ══════════════════════════════════════════════════════════════
//  ATHX Epicure — Auth client-side
//  Protection par mot de passe + session sessionStorage
// ══════════════════════════════════════════════════════════════

const AUTH_SESSION_KEY = "athx_auth_session_v1";
const AUTH_COMP_KEY    = "athx_auth_comp_v1";   // comp lié à la session
...
async function getStoredHash() {
  try {
    // AthxStore doit être chargé avant auth.js
    if (typeof AthxStore === "undefined") return null;
...
function isAuthenticated() {
  try {
    ...
    const comp = new URLSearchParams(window.location.search).get("comp") || "default";
...
function requireAuth(options = {}) {
  ...
  function inject() {
    ...
    overlay.innerHTML = `
      <div id="auth-modal">
        <div id="auth-logo">ATHX · EPICURE</div>
        <div id="auth-lock">🔒</div>
        <div id="auth-title">Zone réservée</div>
...
// Exporter
window.AthxAuth = { requireAuth, login, logout, isAuthenticated, changePassword, hashPassword };
